...
| 41 | [Agente de reporte de voz — orquestador + subagentes](41-voice-report-agent.md) | 📋 | Orquestador pydantic-ai (Groq, en-stack) con subagentes-como-tools `collect_inbox`/`summarize`/`write_script` (loop ReAct); el recolector es agéntico a su vez. Patrón elegido para hacer atribuibles las 6 métricas de Plan 42 (tool-calls + iterations). `POST /reports/voice` → `VoiceScript` con buen copy. v1 solo guion (`audio_url=None`). Habilita Plan 42 |
...
| 44 | [Copiloto de tuning de triage — orquestador](44-triage-tuning-copilot.md) | 📋 | La feature genuinamente agéntica: dada una triage mal clasificada, un orquestador diagnostica (Plan 43) → propone cambios al **borrador** (few-shot/categoría vía Plan 26/27) → corre evals → itera hasta pasar el eval-gate. `POST /workspaces/{tid}/tune` → `TuningProposal`; **el publish lo hace el humano**. Sujeto principal de las 6 métricas (Plan 42) |
