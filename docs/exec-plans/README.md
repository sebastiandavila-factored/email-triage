...
| 48 | [CI de tests (unit/e2e) + seguridad (Level 4)](48-ci-testing-and-security.md) | 🚧 | Workflow de tests con matrix **unit vs e2e** (split auto-marcado por fixture en `conftest.py`: 154 unit / 106 e2e), + **pip-audit** + **SBOM CycloneDX** + **Dependabot**. Incluye la remediación de 10 CVEs (pydantic-ai acotado a `<2`; starlette≥1.3.1 → `httpx2.Response` en 2 tests). Tooling/CI; rama `feat/add_CI_for_testing` |
| 49 | [CI de frontend — gates + Vitest + Playwright](49-frontend-ci-testing.md) | 🚧 | `frontend-ci.yml` path-filtered a `frontend/**`: jobs lint/build/**Vitest unit**/**Playwright e2e** (API mockeada, sin backend). Node atado vía `engines`+`.nvmrc`+`node-version-file` para matchear Vercel. En 3 commits. Rama `feat/frontend_CI` |
