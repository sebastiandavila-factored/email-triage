...
| Failure mode | 500 or a crash | Real cause logged to Logfire; caller gets a semantic 503 + correlation id — legacy fallback only for the intended no-config default, never to mask an outage |
...
| **Fail loud on a genuine failure; default quietly on an intended state** | Some uptime when the infra is down | A tenant that hasn't configured a taxonomy yet is served the legacy default — the intended state, not an error. But a *real* failure (DB unreachable, an unreadable prompt version, a deprecated model) is never masked as a successful triage on the wrong taxonomy: the real cause goes to Logfire and the caller gets a semantic 503 with a correlation id. A confidently-wrong classification is worse than an honest "retry shortly." |
